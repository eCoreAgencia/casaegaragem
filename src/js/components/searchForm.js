	...
	$.fn.searchform = function(options) {
			...
			items.forEach(item => {
				...
				vtexjs.catalog
					...
					.done(function(data) {
						...
						let bestPriceFormated = formatter.format(
							parseFloat(
						...
						let listPriceFormated = formatter.format(
							parseFloat(
						...
						let available = data.skus[0].available;
								...
								<div class="product__price">
									<div class="price">
										...
										<span class="price__instament">${
											available && parcelas > 1
												? `${parcelas}x`
												: ""
										} ${
								available && parcelas > 1
									? `R$ ${valorParcela} sem juros`
									: ""
							}</span>
									</div>
								</div>
		...
		self.searchformMountBrandsList = function(brands, query) {
			...
			brands.forEach(brand => {
				let url = `/${encodeURIComponent(brand).replace(
					".",
					"-"
				)}/${query}`;

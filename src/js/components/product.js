...
import { addToCart, formatter } from "../utils";
	...
	if ($("body").hasClass("product")) {
			...
			vtexjs.catalog
				...
				.then(function(product) {
					...
					<div class="price">
						${
							sku.listPrice > 0
								? '<span class="price__old"> R$ ' +
								  (sku.listPrice / 100).formatMoney() +
								  "</span>"
								: ""
						}
						...
						${
							sku.listPrice > 0
								? '<span class="price__economy"> (Economia de R$ ' +
								  (
										(sku.listPrice - sku.bestPrice) /
										100
								  ).formatMoney() +
								  ")</span>"
								: ""
						...
						${
							sku.installments > 0
								? '<span class="price__installments"> em até <strong>' +
								  sku.installments +
								  "x</strong> de <strong> R$ " +
								  (sku.installmentsValue / 100).formatMoney() +
								  "</strong> s/ juros</span>"
								: ""
						}
						<span class="price__boleto"> ou <strong>R$ ${(
							(sku.bestPrice / 100) *
							0.9
						).formatMoney()}</strong> no boleto </span>
					</div>`;
					if (!product.available) {
						$(".product__main").addClass("product--unavailable");
						html = "Indisponível";
					}
		...
		class Product {
			constructor() {
				...
				if ($("td.buy")[0]) {
					const regex = /Valor total:.*/gim;
					const str = $("td.buy").html();
					let m = str.match(regex);
					console.log(m);
					m = m[0].replace(
						"Valor total:  R$ ",
						"Compre todos por  <strong>R$ "
					);

					const price =
						'<span class="price-cash">' + m + "</strong></span>";
				...
				if ($(".flag.leve-mais-pague-menos")[0]) {
					...
					<div class="buy-more__item">
						<span class="buy-more__text" >Leve ${
							item.leve
						} e pague <strong>${formatter.format(
								skuPrice - skuPrice * item.pague
							)}</strong>cada.</span>
					</div>`;
				...
				$("body").on("click", function(e) {
					...
					const buyMore = document.querySelector(
